...
private void addListeners() {
    searchTextField.getDocument().addDocumentListener(new DocumentListener() {
        ...
        // Fungsi untuk menjalankan pencarian berdasarkan input pengguna
        private void executeSearch() {
            String keyword = searchTextField.getText().trim();
            if (keyword.isEmpty()) {
                loadTableData(); // Jika tidak ada input, tampilkan semua data
            } else {
                searchAddressBook(keyword); // Pencarian berdasarkan keyword
            }
        }
    });
}
...
private void addData() {
    if (validateInput()) {
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            refreshTable(); // Refresh tabel setelah data ditambahkan
...
private void updateData() {
    if (validateInput() && !idTextField.getText().trim().isEmpty()) {
        ...
        try (Connection conn = DatabaseConnection.connect();
            ...
            refreshTable(); // Refresh tabel setelah data diubah
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    } else {
...
private boolean validateInput() {
    ...
    try {
        Long.parseLong(phoneTextField.getText().trim()); // Validasi nomor telepon berupa angka
...
// Fungsi untuk pencarian data di AddressBook
private void searchAddressBook(String keyword) {
    ...
    DefaultTableModel model = (DefaultTableModel) jTable.getModel();
    model.setRowCount(0); // Kosongkan tabel sebelum memuat data pencarian
    try (Connection conn = DatabaseConnection.connect();
         PreparedStatement pstmt = conn.prepareStatement(sql)) {
        String searchKeyword = "%" + keyword + "%"; // Wildcard pencarian
...
// Fungsi untuk mengekspor data ke file teks
private void exportToFile() {
    ...
    try (Connection conn = DatabaseConnection.connect();
        ...
        writer.newLine();
        while (rs.next()) {
            writer.write(rs.getInt("id") + ", " + rs.getString("name") + ", " + rs.getString("address") + ", " + rs.getString("phone"));
            writer.newLine();
        }
        writer.flush();
        JOptionPane.showMessageDialog(this, "Data berhasil diekspor ke " + filePath, "Export Success", JOptionPane.INFORMATION_MESSAGE);
    } catch (IOException | SQLException ex) {
        ex.printStackTrace();
    }
}

// Fungsi untuk mengimpor data dari file teks
private void importFromFile() {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(filePath));
         ...
         PreparedStatement pstmt = conn.prepareStatement(sql)) {
        reader.readLine(); // Lewati header file
        String line;
        ...
        refreshTable(); // Refresh tabel setelah data diimpor
        JOptionPane.showMessageDialog(this, "Data berhasil diimpor dari " + filePath, "Import Success", JOptionPane.INFORMATION_MESSAGE);
    } catch (IOException | SQLException ex) {
        ex.printStackTrace();
    }
}
...
// Fungsi tambahan untuk memuat ulang tabel
private void refreshTable() {
    clearFields(); // Kosongkan field
